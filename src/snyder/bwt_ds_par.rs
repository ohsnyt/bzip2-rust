...
pub fn bwt_encode_par(data: &[u8]) -> (u32, Vec<u8>) {
    info!("Entering parallel BTW sorting algorithm.");
...
fn par_bwt_sort(data: &mut [BwtKey]) {
...
fn par_subsorting(data: &mut [BwtKey], rundepth: u32, udata: &Vec<usize>) -> bool {
    ...
    //});
    seqs.iter().for_each(|(start, end)| {
        update_btw_keys(&mut data[*start..*end], rundepth, udata);
...
fn update_btw_keys(data: &mut [BwtKey], depth: u32, udata: &Vec<usize>) {
    let end = udata.len();
    data.iter_mut().for_each(|el| {
        el.sort = udata[(el.index as usize + (std::mem::size_of::<usize>()) * depth as usize) % end]
            as usize;
        el.depth = depth as u16;
    });
}
